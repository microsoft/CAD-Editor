...
class CADparser:
    ...
    def perform(self, cad_seq):
        # Check for floating point numbers or non-integers in the input string  
        if re.search(r'\d+\.\d+', cad_seq):  
            print("contain float nums!")  
            return None
        # divide into sketch and extrude
        sketches, extrudes = self.get_SE(cad_seq)
        if sketches is None or extrudes is None:
            print("sketches is None or extrudes is None!") 
            return None
    ...
    def get_SE(self, cad_seq):
        ...
        if len(sketches) != len(extrudes):
            print("len(sketches) != len(extrudes)")
            return None, None
    ...
    def dequantize_line_points(self, curve, next_curve, scale, offset):
        ...
        try:
            point = np.array(next_curve[1:3], dtype=int)
            end_v = dequantize_verts(
            point,
            n_bits=self.bit,
            min_range=-SKETCH_R,
            ...
            print(f"Invalid data in next_curve: {next_curve[1:3]}")
            return None, None
    ...
    for idx, item in enumerate(data):
        try:
            ...
            if args.type == "original":
                ...
                parsed_data = cad_parser.perform(item["output_sequence"])

            out_path = os.path.join(args.out_path, str(str(idx).zfill(5)) + '_' + str(item['original_pic_name']) + '_' +  str(item['edited_pic_name']))
            #out_path = os.path.join(args.out_path, str(str(idx).zfill(5)))
            ...
            continue  # Skip this item if an error occurs
