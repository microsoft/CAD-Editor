...
class CLIPLoss(torch.nn.Module):
    ...
    def get_text_features(self, instruction_str: str, norm: bool = True) -> torch.Tensor:
        
        tokens = clip.tokenize(instruction_str).to(self.device)
    ...
    def compute_text_direction_instruction(self, instruction: str) -> torch.Tensor:  
        text_direction = self.get_text_features(instruction)  
        text_direction /= text_direction.norm(dim=-1, keepdim=True)
    ...
    def clip_directional_loss_instruction(self, src_img: torch.Tensor, instruction: str, target_img: torch.Tensor) -> torch.Tensor:
    
        if self.target_direction is None:
            self.target_direction = self.compute_text_direction_instruction(instruction)
