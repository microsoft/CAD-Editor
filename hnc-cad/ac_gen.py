...
def sample(args):
    ...
    sketch_dec = SketchDecoder(args.mode, num_code=9390) 
    ...
    count = 0
    for name, pixel_p, coord_p, sketch_mask_p, ext_p, ext_mask_p, _, _, _, _, _, _, _ in dataloader:
        ...
        try:
            ...
            save_folder = os.path.join(result_folder, str(name[0]) + '_' + str(count).zfill(6)+'_origInput')
            if not os.path.exists(save_folder):
                os.makedirs(save_folder)
            write_obj_sample(save_folder, cad_obj)
        except Exception as error_msg: 
            print("error1: ", error_msg) 
            continue 
        ...
        for ii, sample in enumerate(cad_samples):
            try:
                ...
                save_folder = os.path.join(result_folder, str(name[0]) + '_' + str(count).zfill(6) +'_postAC'+str(ii))
                if not os.path.exists(save_folder):
                    os.makedirs(save_folder)
                write_obj_sample(save_folder, cad_obj)
            except Exception as error_msg:  
                print("error2: ", error_msg) 
                continue
        count += 1
        print("count1: ",count)
